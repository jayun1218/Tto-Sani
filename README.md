...
- **AI 소비 페르소나**: OpenAI GPT-4o 분석을 통해 "지름신 부엉이", "철벽 다람쥐" 등 7가지 소비 페르소나를 부여하고 캐릭터의 대사와 반응이 실시간으로 변화.
- **감정 태그 기반 인사이트**: 지출 시의 기분(행복, 우울, 분노 등)을 기록하여 심리 상태와 소비 패턴의 상관관계를 분석하는 고도화된 인사이트 제공.
- **미래 지출 예측**: 현재 소비 속도를 분석해 이번 달 말 총 지출액을 미리 예측하고 상태(정상/경고) 시각화.
- **소비 트렌드 리포트**: 전월 및 전년 동월 대비 지출 변화를 차트로 한눈에 비교 분석.

### 2. 스마트 입력 & 구독 매니저 (Smart Input & Subscriptions)
- **영수증 OCR 자동 입력**: OpenAI GPT-4o Vision을 활용하여 영수증 사진 한 장으로 구매처, 금액, 날짜, 카테고리를 자동으로 추출하여 입력.
...
### 3. 하이브리드 인텔리전스 (Hybrid Intelligence)
...
- **위치 기반 실시간 소비 경고 (LBS)**: 과소비 위험 상권(카페거리, 백화점 등) 진입 시 캐릭터가 "또 사니?"라며 나타나 충동 구매 방지.
